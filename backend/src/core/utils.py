...
from src.core.skills.builtin import register_builtin_skills
...
def search_knowledge_base(
    ...
    - "Local knowledge" or "Knowledge base"
    - Specific details that might be in the provided context
...
@tool(description="DuckDuckGo Search Tool")
async def duckduckgo_search(
    ...
    for query in queries:
        try:
            ...
            res = await asyncio.to_thread(search.run, query)
...
async def get_search_tool(search_api: SearchAPI):
    ...
    if search_api == SearchAPI.ANTHROPIC:
        ...
        # Fallback to DuckDuckGo if TAVILY_API_KEY is not set
        if not get_tavily_api_key(None) and not os.getenv("TAVILY_API_KEY"):
             logging.warning("TAVILY_API_KEY not found, falling back to DuckDuckGo")
...
async def get_all_tools(config: RunnableConfig):
    ...
    """
    # 1. Register builtin skills (idempotent)
    register_builtin_skills()
    
    # 2. Convert all registered skills to tools
    tools = []
    ...
    # Add ResearchComplete and think_tool (special meta-tools)
    tools.extend([tool(ResearchComplete), think_tool])
    
    # Add MCP tools if configured
    configurable = Configuration.from_runnable_config(config)
    mcp_tools = await load_mcp_tools(config, {t.name for t in tools})
