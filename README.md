...
easy_deepresearch 是一个基于 LangGraph 的多智能体深度研究系统：能够拆解复杂问题、并发检索互联网与本地知识库（RAG），并输出结构化、有引用的研究报告。

更完整的技术文档见 [TECHNICAL_DOCUMENTATION.md](file:///d:/LLMLearning/open_deep_research-main/easy_deepresearch/docs/TECHNICAL_DOCUMENTATION.md)。
...
sequenceDiagram
  ...
  G->>S: 规划研究
  S->>R: 并发派发子任务
  R->>T: tavily_search / duckduckgo / search_knowledge_base
  T-->>R: 检索结果与来源
  R-->>S: 子任务结论与证据
  S-->>B: 汇总生成最终报告
  B-->>F: SSE 持续推送事件与报告
...
- 代码入口
  ...
  - 工具注册：[utils.py](file:///d:/LLMLearning/open_deepresearch-main/easy_deepresearch/backend/src/core/utils.py)
...
### Skill System
- **动态注册**: 所有工具（包括搜索、RAG、arXiv）均封装为 `Skill` 对象，通过 `SkillRegistry` 统一管理。
- **可插拔**: 开发者可以轻松编写新的 Python Skill 并注册，Agent 会自动发现并根据语义选择使用。
- **管理界面**: 前端提供技能开关，用户可针对特定任务启用/禁用某些能力（如“仅使用 arxiv 搜索”）。
